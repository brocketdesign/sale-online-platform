...
interface Props {
  reviews: Review[]
  avg: number
  /** When true, hides the large average score column (use in narrow sidebars) */
  compact?: boolean
}

export default function RatingBreakdown({ reviews, avg, compact = false }: Props) {
  ...
  return (
    <div className={compact ? '' : 'flex gap-8 items-start'}>
      {/* Overall score — hidden in compact mode */}
      {!compact && (
      ...
      {/* Bars */}
      <div className={compact ? 'space-y-2' : 'flex-1 space-y-2'}>
        {counts.map(({ star, pct }) => (
          <div key={star} className="flex items-center gap-2">
            <span className="text-sm text-gray-600 w-10 flex items-center gap-0.5 flex-shrink-0">
